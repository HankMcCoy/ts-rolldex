...
beforeEach(async () => {
	...
	await db.insert(users).values([
		{ id: dmId, name: "DM", email: `${dmId}@test.invalid`, emailVerified: true, createdAt: now, updatedAt: now },
		{ id: memberId, name: "Member", email: `${memberId}@test.invalid`, emailVerified: true, createdAt: now, updatedAt: now },
		{ id: outsiderId, name: "Outsider", email: `${outsiderId}@test.invalid`, emailVerified: true, createdAt: now, updatedAt: now },
	]);
...
afterEach(async () => {
	...
	await db.delete(campaigns).where(like(campaigns.name, `Test Campaign ${TEST_PREFIX}%`));
...
describe("getCampaignAccess", () => {
	it("returns ADMIN for the campaign creator", async () => {
		const access = await getCampaignAccess(campaignId, dmId, `${dmId}@test.invalid`);
		expect(access).toBe("ADMIN");
	});

	it("returns READ_ONLY for a member matched by userId", async () => {
		const access = await getCampaignAccess(campaignId, memberId, `${memberId}@test.invalid`);
	...
	it("returns NONE when email matches but member row belongs to a different userId", async () => {
		...
		const access = await getCampaignAccess(campaignId, outsiderId, `${memberId}@test.invalid`);
		expect(access).toBe("NONE");
	});

	it("returns NONE for a non-member", async () => {
		const access = await getCampaignAccess(campaignId, outsiderId, `${outsiderId}@test.invalid`);
		expect(access).toBe("NONE");
	});

	it("returns NONE for a non-existent campaign", async () => {
		const access = await getCampaignAccess("no-such-campaign", dmId, `${dmId}@test.invalid`);
		expect(access).toBe("NONE");
	});
});
...
describe("requireCampaignAccess", () => {
	it("returns ADMIN for the creator with no minimum specified", async () => {
		const access = await requireCampaignAccess(campaignId, dmId, `${dmId}@test.invalid`);
		expect(access).toBe("ADMIN");
	});

	it("returns READ_ONLY for a member with no minimum specified", async () => {
		const access = await requireCampaignAccess(campaignId, memberId, `${memberId}@test.invalid`);
		expect(access).toBe("READ_ONLY");
	});

	it("throws 404 for NONE access", async () => {
		await expect(
			requireCampaignAccess(campaignId, outsiderId, `${outsiderId}@test.invalid`)
		).rejects.toMatchObject({ status: 404 });
	});

	it("throws 403 when ADMIN is required but user is READ_ONLY", async () => {
		await expect(
			requireCampaignAccess(campaignId, memberId, `${memberId}@test.invalid`, "ADMIN")
		).rejects.toMatchObject({ status: 403 });
	});

	it("returns ADMIN when ADMIN is required and user is creator", async () => {
		const access = await requireCampaignAccess(campaignId, dmId, `${dmId}@test.invalid`, "ADMIN");
		expect(access).toBe("ADMIN");
	});
});
